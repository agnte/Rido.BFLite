...
public class UserTokenClient(
    ...
    IConfiguration configuration) : IUserTokenClient
{
    private readonly ILogger<UserTokenClient> _logger = logger;
    private readonly string _apiEndpoint = configuration["ApiEndpoint"] ?? throw new InvalidOperationException("ApiEndpoint not configured");
    ...
    public async Task<GetTokenResult> GetTokenAsync(string userId, string connectionName, string channelId, string? code = null)
    {
        ...
        var result = JsonSerializer.Deserialize<GetTokenResult>(resJson, _defaultOptions)!;
        return result;
    }

    public async Task<GetSignInResourceResult> GetTokenOrSignInResource(string userId, string connectionName, string channelId, string? finalRedirect = null)
    {
        var queryParams = new Dictionary<string, string?>
        {
            { "userid", userId },
        ...
        var result = JsonSerializer.Deserialize<GetSignInResourceResult>(json, _defaultOptions)!;
        return result;
    }

    public async Task<GetTokenStatusResult> GetTokenStatusAsync(string userId, string channelId, string? include = null)
    {
        var queryParams = new Dictionary<string, string?>
        {
            { "userid", userId },
        ...
        var result = JsonSerializer.Deserialize<IList<GetTokenStatusResult>>(json, _defaultOptions)!;
        return result[0]!;
    ...
    private async Task<string> CallApiAsync(string endpoint, Dictionary<string, string?> queryParams, HttpMethod? method = null, string? body = "")
    {
        try
        {
            var authHeader = await authorizationHeaderProvider.CreateAuthorizationHeaderForAppAsync(_scopes);
